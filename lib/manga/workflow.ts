...
export async function createStoryPitches(state: WorkflowState) {
  const wrapper = await structuredResponse({ instructions:'You are an educational comic story editor. Create exactly five structurally different six-beat pitches. Math must drive the action. Never humiliate a child for mistakes.', prompt:JSON.stringify({problem:state.sourceProblem,math:state.mathAnalysis,language:state.language}), name:'story_pitches', schema:pitchJsonSchema, validate:z.object({ pitches: storyPitchSchema.array().length(5) }) })
