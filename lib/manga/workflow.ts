...
const pitchJsonSchema = { type:'object', additionalProperties:false, required:['pitches'], properties:{ pitches:{type:'array',minItems:5,maxItems:5,items:{type:'object',additionalProperties:false,required:['id','type','title','hook','explicitTask','targetScope','observationPoint','successCriteria','reasonToFindAll','ruleContext','synopsis','mathIntegration','beats','interaction','tone','recommendedTraits','whyItFits','riskNotes'],properties:{id:{type:'string'},type:{type:'string',enum:['funny','warm','interactive','tiktok','instagram']},title:{type:'string'},hook:{type:'string',minLength:80},explicitTask:{type:'string'},targetScope:{type:'string'},observationPoint:{type:'string'},successCriteria:{type:'string'},reasonToFindAll:{type:'string'},ruleContext:{type:'string'},synopsis:{type:'string',minLength:200},mathIntegration:{type:'string',minLength:150},beats:{type:'array',minItems:8,maxItems:18,items:{type:'string',minLength:40}},interaction:{type:'string',minLength:60},tone:{type:'string'},recommendedTraits:{type:'array',items:{type:'string'}},whyItFits:{type:'string',minLength:100},riskNotes:{type:'array',items:{type:'string'}}}}}} }
