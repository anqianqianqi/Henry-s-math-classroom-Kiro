...
export async function createStoryPitches(state: WorkflowState) {
  const wrapper = await structuredResponse({ instructions:`You are an educational comic story editor. Create exactly five structurally different pitches. Choose 6–18 story beats for each pitch based on what a first-time learner needs; never force the story into six panels. Clarity, context, mathematical understanding and fun are more important than panel count. Math must drive the action. Never humiliate a child for mistakes. Follow this permanent house preference:\n${mangaPreferencePrompt()}`, prompt:JSON.stringify({problem:state.sourceProblem,math:state.mathAnalysis,language:state.language,renderSpec:state.renderSpec}), name:'story_pitches', schema:pitchJsonSchema, validate:z.object({ pitches: storyPitchSchema.array().length(5) }) })
