...
export async function POST(request:NextRequest){
  try{
    await requireMangaAdmin();const input=saveSchema.parse(await request.json());const db=mangaServiceDb()
...
export async function GET(request:NextRequest){
  try{await requireMangaAdmin();const {data,error}=await mangaServiceDb().from('manga_characters').select('id,name,status,current_version,thumbnail_url,updated_at').neq('status','archived').order('updated_at',{ascending:false});if(error)throw error;return NextResponse.json(data)}
